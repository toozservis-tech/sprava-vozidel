...
import { ensureTestUser, getTestCredentials } from './helpers';
...
test.describe('Authenticated shell visibility', () => {
  test.beforeEach(async ({ page }) => {
    ...
    await ensureTestUser(page, credentials.email, credentials.password);
    const loginResponse = await page.request.post('/user/login', {
      data: {
    ...
    expect(loginPayload?.user?.email || loginPayload?.email).toBeTruthy();
    ...
    await page.addInitScript(({ token, userData }) => {
      ...
      token: String(loginPayload.access_token),
      userData: user,
    });

    await page.goto('/web/app/u/e2e-shell/dashboard');
